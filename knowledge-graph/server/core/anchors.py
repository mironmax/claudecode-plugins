...
Two halves, split by cost:
  ...
  offers the rename git recorded, or the ONE file in the project tree with the
  same basename. Two same-named files is a refusal, not a coin toss — a wrong
  anchor is worse than a dangling one, because it looks right.
"""
    ...
    for entry in entries:
        ...
        if len(matches) > 1:
            ...
            rec.update(verdict="gone",
                       evidence=git_ev if git_ev.startswith("deleted") else
                       f"nothing named {base} anywhere in the project{extra}")
    return out
