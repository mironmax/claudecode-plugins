...
CAVEATS = [
    ...
    "Injected-then-endorsed counts only sessions still active once the "
    "endorsement log had its first record; sessions that ended before it "
    "could not have logged an endorsement.",
...
def _describe(recall: list[dict], useful: list[dict], logged_sessions: set) -> dict:
    ...
    # the session unseen, and the session endorsed afterwards. Sessions over
    # before the endorsement log began could never earn one, so their
    # injections are counted apart instead of diluting the rate.
    first_injected: dict[tuple, float] = {}
    before_log = 0
    for r in recall:
        if r.get("reason") != "injected":
            continue
        if r.get("kg_session") not in logged_sessions:
            before_log += 1
            continue
    ...
    return {
        ...
        "injected_before_log": before_log,
...
def describe(recall: list[dict], useful: list[dict], logged_sessions: set) -> dict:
    """Overall and per-project descriptive block. logged_sessions are the
    kg sessions still active once the endorsement log began."""
    ...
    return {
        "overall": _describe(recall, useful, logged_sessions),
        "projects": {p: _describe(by_proj_r[p], by_proj_u[p], logged_sessions) for p in projects},
    }
