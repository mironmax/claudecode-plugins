...
def main():
    ...
    # Months later an unrelated file takes the freed name elsewhere.
    touch(repo, "notes/b.md", "nothing to do with the old one\n")
    git(repo, "add", "-A")
    ...
    check("a plain delete in git outranks the one same-named file (no replacement)",
          r["replacement"] is None, r)
    r = recs["x/c.py"]
